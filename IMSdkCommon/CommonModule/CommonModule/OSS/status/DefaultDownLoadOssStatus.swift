...
class DefaultDownLoadOssStatus: DownLoadOssBaseStatus {
    ...
    override func startDownload(objectId: String) -> Promise<OssDownLoadStatus> {
        ...
        return IMFileProgressDao(db: dataBase).queryWithObjectId(objectId: objectId)
            .then { progressModel -> Promise<Int> in
                ...
                return IMFileProgressDao(db: dataBase).updateProgress(objectId: objectId, progress: lp, timeStamp: Date().milliStamp, retryCount: 0).then { _ -> Promise<Int> in
