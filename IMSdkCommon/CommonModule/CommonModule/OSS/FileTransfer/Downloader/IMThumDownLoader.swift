...
class IMThumDownLoader: IMDownloader {
...
//                            })
                        ...
                        if transferInfo.fileStatus == IMFileTransferStatus.Downloading_NoKey {
                            ...
                            DispatchQueue.main.async {
                                //{cropUrl}{objectId}_{width}_{height}_{type}.{ext}&bucket={bucketName}&ver=2
                                let url = (value.cropHost ?? "") + objectID + "&ver=2&bucket=" + (value.bucketName ?? "")
                                IMAwsService.downloadApi(url: url, context: self.ossContext)
                                    .then { (str) -> Promise<Void> in
...
//                                                })
                                            ...
                                            //}else if transferInfo.fileStatus == IMFileTransferStatus.Download_Failed || transferInfo.fileStatus == IMFileTransferStatus.Downloading_NoKey {
                                        ...
                                        // just special error with aws3, update progress value 404
                                        let errorProgress: Int = IMTransferProgressState.failedMin.rawValue
                                        IMFileProgressDao(db: dataBase).updateProgressOnError(objectId: objectID, progress: errorProgress).then({ Void -> Promise<Void> in
                                            ...
                                            fileDownloadEvent.objectIds = [objectID]
                                            notificationManager.post(eventProtocol: fileDownloadEvent)
                                            self.oss?.deleagte?.updateFileBusinessFailed(objectId: objectID, progress: errorProgress)
                                            return Promise<Void>.resolve()
                                        })
