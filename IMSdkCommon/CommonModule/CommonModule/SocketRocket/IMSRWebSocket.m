...
@interface NSData (TMMSRWebSocket)
...
@interface NSString (TMMSRWebSocket)
...
@interface NSURL (TMMSRWebSocket)
...
@interface _TMSRRunLoopThread : NSThread
...
@implementation NSData (TMMSRWebSocket)
...
@implementation NSString (TMMSRWebSocket)
...
NSString *const IMSRWebSocketErrorDomain = @"IMSRWebSocketErrorDomain";
NSString *const IMTMSRWebSocketErrorDomain = @"HTTPResponseStatusCode";

// Returns number of bytes consumed. Returning 0 means you didn't match.
// Sends bytes to callback handler;
typedef size_t (^stream_scanner)(NSData *collected_data);

typedef void (^data_callback)(IMSRWebSocket *webSocket,  NSData *data);

@interface TMSRIOConsumer : NSObject {
...
@interface TMSRIOConsumerPool : NSObject

- (id)initWithBufferCapacity:(NSUInteger)poolSize;

- (TMSRIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
- (void)returnConsumer:(TMSRIOConsumer *)consumer;

@end

@interface IMSRWebSocket ()  <NSStreamDelegate>

@property (nonatomic) IMSRReadyState readyState;
...
@implementation IMSRWebSocket {
    ...
    __strong IMSRWebSocket *_selfRetain;
    
    NSArray *_requestedProtocols;
    TMSRIOConsumerPool *_consumerPool;
}
...
- (void)_SR_commonInit;
{
    ...
    _consumerPool = [[TMSRIOConsumerPool alloc] init];
...
- (void)setReadyState:(IMSRReadyState)aReadyState;
{
...
{
    assert(_url);
    NSAssert(_readyState == IMSR_CONNECTING, @"Cannot call -(void)open on TMMSRWebSocket more than once");
    ...
    if (_urlRequest.timeoutInterval > 0)
    {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            if (self.readyState == IMSR_CONNECTING)
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if (responseCode >= 400) {
        ...
        [self _failWithError:[NSError errorWithDomain:IMSRWebSocketErrorDomain code:2132 userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"received bad response code from server %ld", (long)responseCode], IMTMSRWebSocketErrorDomain:@(responseCode)}]];
        return;
    }
    
    if(![self _checkHandshake:_receivedHTTPHeaders]) {
        [self _failWithError:[NSError errorWithDomain:IMSRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid Sec-WebSocket-Accept response"] forKey:NSLocalizedDescriptionKey]]];
        return;
    }
    ...
    if (negotiatedProtocol) {
        ...
        if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
            [self _failWithError:[NSError errorWithDomain:IMSRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Server specified Sec-WebSocket-Protocol that wasn't requested"] forKey:NSLocalizedDescriptionKey]]];
            return;
        }
...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(IMSRWebSocket *self,  NSData *data) {
...
- (void)_updateSecureStreamOptions;
{
    if (_secure) {
        ...
        if ([_urlRequest TMMSR_SSLPinnedCertificates].count) {
...
- (void)close;
{
    [self closeWithCode:IMSRStatusCodeNormal reason:nil];
}
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason;
{
    ...
    dispatch_async(_workQueue, ^{
        if (self.readyState == IMSR_CLOSING || self.readyState == IMSR_CLOSED) {
            return;
        }
        
        BOOL wasConnecting = self.readyState == IMSR_CONNECTING;
        
        self.readyState = IMSR_CLOSING;
...
- (void)_closeWithProtocolError:(NSString *)message;
{
    ...
    [self _performDelegateBlock:^{
        [self closeWithCode:IMSRStatusCodeProtocolError reason:message];
        dispatch_async(_workQueue, ^{
            [self closeConnection];
        });
    }];
...
{
    dispatch_async(_workQueue, ^{
        if (self.readyState != IMSR_CLOSED) {
            _failed = YES;
            [self _performDelegateBlock:^{
...
- (void)send:(id)data;
{
    NSAssert(self.readyState != IMSR_CONNECTING, @"Invalid State: Cannot call send: until connection is open");
...
{
    NSAssert(self.readyState == IMSR_OPEN, @"Invalid State: Cannot call send: until connection is open");
    // TODO: maybe not copy this for performance
...
- (void)handleCloseWithData:(NSData *)data;
{
    ...
    if (dataSize == 1) {
        ...
        _closeCode = IMSRStatusNoStatusReceived;
    }
    
    [self assertOnWorkQueue];
    
    if (self.readyState == IMSR_OPEN) {
        ...
        case SROpCodeTextFrame: {
            if ([self.delegate respondsToSelector:@selector(webSocketShouldConvertTextFrameToString:)] && ![self.delegate webSocketShouldConvertTextFrameToString:self]) {
                ...
                if (str == nil && frameData) {
                    [self closeWithCode:IMSRStatusCodeInvalidUTF8 reason:@"Text frames must be valid UTF-8"];
                    dispatch_async(_workQueue, ^{
                        [self closeConnection];
                    });
                    return;
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(IMSRWebSocket *self, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(IMSRWebSocket *self, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [self _addConsumerWithDataLength:extra_bytes_needed callback:^(IMSRWebSocket *self, NSData *data) {
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        ...
        if (bytesWritten == -1) {
            [self _failWithError:[NSError errorWithDomain:IMSRWebSocketErrorDomain code:2145 userInfo:[NSDictionary dictionaryWithObject:@"Error writing to stream" forKey:NSLocalizedDescriptionKey]]];
             return;
        }
...
- (BOOL)_innerPumpScanner {
    ...
    TMSRIOConsumer *consumer = [_consumers objectAtIndex:0];
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
    ...
    if (!frame) {
        [self closeWithCode:IMSRStatusCodeMessageTooBig reason:@"Message too big"];
        return;
    }
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode;
{
    ...
    if (_secure && !_pinnedCertFound && (eventCode == NSStreamEventHasBytesAvailable || eventCode == NSStreamEventHasSpaceAvailable)) {
        ...
        NSArray *sslCerts = [_urlRequest TMMSR_SSLPinnedCertificates];
        if (sslCerts) {
...
- (void)safeHandleEvent:(NSStreamEvent)eventCode stream:(NSStream *)aStream
{
        switch (eventCode) {
            case NSStreamEventOpenCompleted: {
                ...
                if (self.readyState >= IMSR_CLOSING) {
                    return;
                }
                assert(_readBuffer);
                
                // didConnect fires after certificate verification if we're using pinned certificates.
                BOOL usingPinnedCerts = [[_urlRequest TMMSR_SSLPinnedCertificates] count] > 0;
                if ((!_secure || !usingPinnedCerts) && self.readyState == IMSR_CONNECTING && aStream == _inputStream) {
                    [self didConnect];
                }
                [self _pumpWriting];
                [self _pumpScanner];
                ...
                if (aStream.streamError) {
                    ...
                    dispatch_async(_workQueue, ^{
                        ...
                        if (!_sentClose && !_failed) {
                            ...
                            [self _performDelegateBlock:^{
                                if ([self.delegate respondsToSelector:@selector(webSocket:didCloseWithCode:reason:wasClean:)]) {
                                    [self.delegate webSocket:self didCloseWithCode:IMSRStatusCodeGoingAway reason:@"Stream end encountered" wasClean:NO];
                                }
                            }];
                        }
                    });
...
@implementation TMSRIOConsumerPool {
...
- (TMSRIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
{
    TMSRIOConsumer *consumer = nil;
    if (_bufferedConsumers.count) {
        consumer = [_bufferedConsumers lastObject];
        [_bufferedConsumers removeLastObject];
    } else {
        consumer = [[TMSRIOConsumer alloc] init];
    }
...
@implementation  NSURLRequest (TMMSRCertificateAdditions)

- (NSArray *)TMMSR_SSLPinnedCertificates;
{
    return [NSURLProtocol propertyForKey:@"TMMSR_SSLPinnedCertificates" inRequest:self];
}

@end

@implementation  NSMutableURLRequest (TMMSRCertificateAdditions)
...
- (void)setSR_SSLPinnedCertificates:(NSArray *)TMMSR_SSLPinnedCertificates;
{
    [NSURLProtocol setProperty:TMMSR_SSLPinnedCertificates forKey:@"TMMSR_SSLPinnedCertificates" inRequest:self];
}

@end

@implementation NSURL (TMMSRWebSocket)
...
static _TMSRRunLoopThread *networkThread = nil;
static NSRunLoop *networkRunLoop = nil;

@implementation NSRunLoop (TMMSRWebSocket)

+ (NSRunLoop *)SR_networkRunLoop {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        networkThread = [[_TMSRRunLoopThread alloc] init];
...
@implementation _TMSRRunLoopThread {
