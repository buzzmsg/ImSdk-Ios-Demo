...
//#import "UIImageView+TMWebImage.h"
...
+ (void)saveToPhotoAlbumWithAlbumName:(NSString *)albumName data:(NSData *)data completionBlock:(void (^ _Nullable)(BOOL, NSError * _Nullable))completionBlock{
    [IMPhotoManager getAlbumWithAlbum:albumName completionBlock:^(PHAssetCollection * _Nullable assetCollection) {
        if (assetCollection) {
...
+ (void)compressImagePath:(NSString *)imagePath newSize:(CGSize)newSize complationHandle:(void(^)(NSData *resultData))completed
{
    ...
    dispatch_async(queue, ^{
        UIImage *reImage = [[UIImage alloc] initWithContentsOfFile:imagePath];
        UIImage *resultImage = [self resizedImage:reImage newSize:newSize];
        NSData *data = UIImageJPEGRepresentation(resultImage, 0.5);
        NSLog(@"---clip size:%ld",data.length);
        completed(data);
    });
...
+ (void)compressImage:(UIImage *)sourceImage newSize:(CGSize)newSize complationHandle:(void(^)(NSData *resultData))completed
{
    ...
    dispatch_async(queue, ^{
//        UIImage *reImage = [[UIImage alloc] initWithContentsOfFile:imagePath];
//        UIImage *resultImage = [self resizedImage:sourceImage newSize:newSize];
...
+ (UIImage *)resizedImage:(UIImage *)sourceImage newSize:(CGSize)newSize {
    CGRect newRect = CGRectMake(0, 0, newSize.width, newSize.height);
    UIGraphicsBeginImageContext(newRect.size);
    UIImage *newImage = [[UIImage alloc] initWithCGImage:sourceImage.CGImage scale:1 orientation:sourceImage. imageOrientation];
...
//        NSData *imageData = UIImageJPEGRepresentation(sourceImage, compression);
        ...
        NSUInteger fImageBytes = maxSize;//需要压缩的字节Byte，iOS系统内部的进制1000
...
//                resultImage = [IMPhotoManager thumbnailForData:imageData maxPixelSize:MAX(size.width, size.height)];
