...
@protocol TMTextLinePositionModifier;
 ...
 */
extern const CGSize TMTextContainerMaxSize;

/**
 The TMTextContainer class defines a region in which text is laid out.
 TMTextLayout class uses one or more TMTextContainer objects to generate layouts.
 
 A TMTextContainer defines rectangular regions (`size` and `insets`) or 
 ...
 Example:
     ...
     └─────────────────────────────┘
 */
@interface TMTextContainer : NSObject <NSCoding, NSCopying>
...
/// The constrained size. (if the size is larger than TMTextContainerMaxSize, it will be clipped)
...
@property TMTextTruncationType truncationType;
...
@property (nullable, copy) id<TMTextLinePositionModifier> linePositionModifier;
@end


/**
 The TMTextLinePositionModifier protocol declares the required method to modify
 the line position in text layout progress. See `TMTextLinePositionSimpleModifier` for example.
 */
@protocol TMTextLinePositionModifier <NSObject, NSCopying>
@required
/**
 This method will called before layout is completed. The method should be thread-safe.
 @param lines     An array of TMTextLine.
 @param text      The full text.
 @param container The layout container.
 */
- (void)modifyLines:(NSArray<TMTextLine *> *)lines fromText:(NSAttributedString *)text inContainer:(TMTextContainer *)container;
@end


/**
 A simple implementation of `TMTextLinePositionModifier`. It can fix each line's position
 to a specified value, lets each line of height be the same.
 */
@interface TMTextLinePositionSimpleModifier : NSObject <TMTextLinePositionModifier>
...
/**
 TMTextLayout class is a readonly class stores text layout result.
 ...
 example: (layout with a circle exclusion path)
     ...
     └──────────────────────────┘
 */
@interface TMTextLayout : NSObject <NSCoding>
...
*/
+ (nullable TMTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable TMTextLayout *)layoutWithContainer:(TMTextContainer *)container text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable TMTextLayout *)layoutWithContainer:(TMTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range;

/**
 Generate layouts with the given containers and text.
 
 @param containers An array of TMTextContainer object (if nil, returns nil).
 @param text       The text (if nil, returns nil).
 @return An array of TMTextLayout object (the count is same as containers),
    or nil when an error occurs.
 */
+ (nullable NSArray<TMTextLayout *> *)layoutWithContainers:(NSArray<TMTextContainer *> *)containers
...
///< The text container
@property (nonatomic, strong, readonly) TMTextContainer *container;
///< The full text
...
///< Array of `TMTextLine`, no truncated
@property (nonatomic, strong, readonly) NSArray<TMTextLine *> *lines;
///< TMTextLine with truncated token, or nil
@property (nullable, nonatomic, strong, readonly) TMTextLine *truncatedLine;
///< Array of `TMTextAttachment`
@property (nullable, nonatomic, strong, readonly) NSArray<TMTextAttachment *> *attachments;
 ...
 */
- (nullable TMTextPosition *)closestPositionToPoint:(CGPoint)point;
...
/**
 ...
 @return A text position, or nil if not found.
 */
- (nullable TMTextPosition *)positionForPoint:(CGPoint)point
                                  oldPosition:(TMTextPosition *)oldPosition
                                otherPosition:(TMTextPosition *)otherPosition;
 ...
 */
- (nullable TMTextRange *)textRangeAtPoint:(CGPoint)point;
...
/**
 ...
 at point. Or nil if not found.
 */
- (nullable TMTextRange *)closestTextRangeAtPoint:(CGPoint)point;
 ...
 */
- (nullable TMTextRange *)textRangeByExtendingPosition:(TMTextPosition *)position;
...
/**
 ...
 @return The line index, or NSNotFound if not found.
 */
- (NSUInteger)lineIndexForPosition:(TMTextPosition *)position;
 ...
 */
- (CGPoint)linePositionForPosition:(TMTextPosition *)position;
 ...
 */
- (CGRect)caretRectForPosition:(TMTextPosition *)position;
...
/**
 ...
 If not found, it returns CGRectNull.
 */
- (CGRect)firstRectForRange:(TMTextRange *)range;
...
/**
 ...
 If not found, it returns CGRectNull.
 */
- (CGRect)rectForRange:(TMTextRange *)range;
...
/**
 ...
 @return An array of `TMTextSelectionRect` objects that encompass the selection.
 If not found, the array is empty.
 */
- (NSArray<TMTextSelectionRect *> *)selectionRectsForRange:(TMTextRange *)range;
...
/**
 ...
 If not found, the array is empty.
 */
- (NSArray<TMTextSelectionRect *> *)selectionRectsWithoutStartAndEndForRange:(TMTextRange *)range;
...
/**
 ...
 @param range An object representing a range in text.
 @return An array of `TMTextSelectionRect` objects contains the start and end to
 the selection. If not found, the array is empty.
 */
- (NSArray<TMTextSelectionRect *> *)selectionRectsWithOnlyStartAndEndForRange:(TMTextRange *)range;
...
- (void)drawInContext:(nullable CGContextRef)context
                ...
                debug:(nullable TMTextDebugOption *)debug
