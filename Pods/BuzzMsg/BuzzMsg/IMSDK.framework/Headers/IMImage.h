...
#import "IMFrameImage.h"
#import "IMSpriteSheetImage.h"
#import "IMImageCoder.h"
...
/**
 A IMImage object is a high-level way to display animated image data.
 ...
 animation, try replace UIImageView with `IMAnimatedImageView`.
 
 Sample Code:
 
     // animation@3x.webp
     IMImage *image = [IMImage imageNamed:@"animation.webp"];
     IMAnimatedImageView *imageView = [IMAnimatedImageView alloc] initWithImage:image];
     [view addSubView:imageView];
    
 */
@interface IMImage : UIImage <TMAnimatedImage>

+ (nullable IMImage *)imageNamed:(NSString *)name; // no cache!
+ (nullable IMImage *)imageWithContentsOfFile:(NSString *)path;
+ (nullable IMImage *)imageWithData:(NSData *)data;
+ (nullable IMImage *)imageWithData:(NSData *)data scale:(CGFloat)scale;
